...
messaging.onMessage(function (payload) {
    var count = $('.user_notification_count').data('count');
    count = parseInt(count);
    count++;

    // $('.user_notification_count_view').html(count);
    // $('.user_notification_count_view2').html(count);
    // $('.user_notification_count').data('count',count);
    refresh_notification_data();
    $('.user_notification_count').on('click',function(){
    ...
    // $('.user_notification_detail').prepend(html);
...
function updatecountdatachemist(){
    ...
    //      dataType: "json",
    //      type: "POST",
    //      url: base_url+'chemist/Dashboard/read_user_notification_count',
    //      data: {notify_user_type:'Vendor',user_id:login_user_id},
    //      success: function (data) {
    //          if (data == 100) {
    //              $('.user_notification_count_view').html(0);
    //              $('.user_notification_count').data('count',0);
    //          }
    //      }
    //  });
 }
...
function refresh_notification_data(){
    $.ajax({
        dataType: "json",
        type: "POST",
        url: base_url+'/Account/refresh_notification_data',
        data: {notify_user_type:'Vendor',user_id:login_user_id},
        success: function (data) {
            if (data.status == 1) {
                $('.user_notification_count_view').html(data.result.count);
                $('.user_notification_count_view2').html(data.result.count);
                $('.user_notification_count').data('count',data.result.count);
                var html = '';
                ...
                $('.user_notification_detail').html(html);
            }
        }
    });
}
